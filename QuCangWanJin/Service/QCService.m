...
+ (void)appApiUploadImageWithApi:(NSString *)api
    ...
    // 2. 合并 + RSA 加密参数
    NSMutableDictionary *mergedParam = [[QCServiceUtils mergeRequestParameters:param] mutableCopy];
    mergedParam[@"user_id"] = userModel.user_info.user_id;
    AFHTTPSessionManager *manager = [self sharedAFManager];
    
    [manager POST:requestURL parameters:mergedParam headers:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
